...
import argparse
import sys
module_dir = "/mnt/libingyu/code/U3M"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='/mnt/libingyu/code/U3M/configs/mcubes_rgbadn.yaml')
    args = parser.parse_args()

    with open(args.cfg) as f:
        cfg = yaml.load(f, Loader=yaml.SafeLoader)

    test_file = Path(cfg['EVAL']['IMAGEROOT'])
    ...
    save_dir = Path(cfg['EVAL']['VIS_SAVE_DIR']) / 'test_results'
    ...
    with console.status("[bright_green]Processing..."):
        ...
        segmap = semseg.predict(str(test_file), cfg['EVAL']['OVERLAY'])
        ...
        #         segmap = semseg.predict(str(file), cfg['EVAL']['OVERLAY'])
